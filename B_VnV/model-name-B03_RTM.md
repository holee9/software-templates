...
RTM(Requirements Traceability Matrix)이 없거나 불완전하면 FDA eSTAR 심사에서 첫 번째 deficiency 대상이 된다. FDA eSTAR에는 추적성(Traceability)을 위한 전용 섹션이 존재하며, 심사관은 RTM을 통해 "모든 요구사항이 설계에 반영되었고, 시험으로 검증되었으며, 위험 통제와 연결되었다"는 4방향 추적을 확인한다. RTM 없이는 SRS, SAD, 시험 보고서가 아무리 잘 작성되어 있어도 서로의 연결을 증명할 수 없다. EU MDR의 경우 GSPR(일반 안전성·성능 요건)과 설계의 연결이 RTM을 통해 이루어지므로 CE 마킹의 핵심 근거 문서이기도 하다.

- **FDA**: eSTAR에 추적성 전용 섹션이 있으며, 심사관이 가장 먼저 확인하는 항목 중 하나다. 추적성 불완전 = 심사관의 첫 번째 deficiency 대상. "SRS 요구사항 100개 중 80개만 시험 커버" 수준이면 즉시 AI 요청이 발송된다.
- **MFDS**: 소프트웨어 검증의 완전성을 RTM을 통해 확인한다. RTM 없이는 "모든 요구사항이 검증되었다"는 주장을 뒷받침할 수 없어 허가 심사 보완 대상이 된다.
- **EU MDR**: GSPR과 설계·시험의 연결이 RTM을 통해 이루어진다. Annex II §3.4 및 Annex I GSPR 적합성 증거의 핵심 문서로, CE 마킹 심사의 필수 검토 항목이다.

### 시장별 요구 수준

| 시장 | 요구 수준 | 설명 |
|------|----------|------|
| FDA | 필수 | eSTAR 추적성 전용 섹션 직접 대응. 불완전 시 첫 번째 deficiency 대상. 100% 커버리지 목표 |
| MFDS | 필수 | SW 검증 완전성 확인 수단. RTM 없으면 검증 완전성 주장 불가 |
| EU MDR | 필수 | GSPR 적합성 증거 핵심 문서. Annex I GSPR ↔ 설계 ↔ 시험 ↔ 위험 통제 4방향 연결 필수 |

### 최소 필수 vs 리스크 최소화

- **최소 필수로 작성할 경우**: SRS 요구사항 ID ↔ 시험 케이스 ID(B01/B02)의 2방향 추적을 포함한다. 모든 SRS 요구사항에 최소 1개의 시험 케이스가 연결되어야 하며, 미커버 요구사항은 사유와 처리 계획을 명시해야 한다. 이것만으로 FDA 기본 요건을 충족할 수 있다.
- **리스크 최소화로 확장할 경우**: SRS ↔ SAD ↔ 시험 ↔ 위험 통제(D01)의 4방향 추적을 구현하면 FDA 심사관의 어떤 교차검증 질문에도 즉시 답변할 수 있다. EU MDR GSPR 열을 추가하면 CE 마킹 심사에서 별도 GSPR 적합성 문서 없이도 요건 충족을 증명할 수 있다. Excel, Jira, Confluence, Polarion 등 도구를 활용하여 RTM을 자동 생성하면 SRS 변경 시 즉각적으로 미커버 요구사항을 탐지할 수 있다.

### 외주 개발 시 유의사항

RTM은 외주사가 작성하는 것이 아니라 company-name의 RA 담당자가 주도하여 작성·관리해야 한다. 외주사는 SRS 요구사항 ID와 시험 케이스 ID를 정확히 연결하여 데이터를 제공하는 역할이며, 최종 추적성 검증과 GSPR 매핑은 company-name 책임이다. SOW에 다음을 명시할 것: ① 외주사는 납품 시 시험 케이스와 SRS 요구사항 ID의 대응 표 제출 의무, ② RTM 최종 버전 승인은 company-name RA 담당자가 수행, ③ SRS 변경 시 RTM 업데이트도 동시에 수행 의무.
