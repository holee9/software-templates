...
### 이 문서가 없으면?
라벨링 오류 또는 IFU 누락은 FDA 리콜 사유 1위다. IFU 없이 의료기기를 판매하는 것은 3개 시장 모두에서 불법이다. 2026년 기준 FDA는 사이버보안 구성 정보가 IFU에 포함되지 않은 경우 deficiency letter를 발행하며, EU MDR은 다국어 IFU가 없으면 CE 마킹 자체가 유지되지 않는다. 또한 라벨링은 규제 제출 이후에도 변경 시마다 각 시장에 변경 허가·신고가 필요하므로, 최초부터 올바르게 작성하지 않으면 허가 이후 지속적인 행정 부담이 발생한다.

### 시장별 요구 수준
| 시장 | 요구 수준 | 설명 |
|------|----------|------|
| FDA | 필수 | 21 CFR §801 법적 의무. §524B(2023년)에 따라 사이버보안 구성 정보 IFU 포함 필수. 미포함 시 2026년부터 deficiency letter 발행 |
| MFDS | 필수 | 의료기기법 제20조 및 표시기재 규정에서 한국어 IFU 및 라벨 의무화. 한국어 미제공 시 수입 통관 불가 |
| EU MDR | 필수 | Annex I §23에서 라벨·IFU 기재 사항을 목록으로 규정. 다국어(판매국 언어) 필수. CE 마킹 불가 |

### 최소 필수 vs 리스크 최소화
- **최소 필수**: 제품명·모델명, 제조사 정보(company-name), 의도된 사용, 금기사항, 경고·주의 사항, 보관 및 취급 방법, 기술 지원 연락처. 이 항목들이 누락되거나 부정확하면 각 시장 접수 심사에서 즉각 보완 요청이 발행된다.
- **리스크 최소화**: 사이버보안 구성 지침(네트워크 요구 사항, 패치 업데이트 절차, 사고 신고 방법)을 IFU에 포함하면 FDA §524B 및 EU MDR Annex I §21을 동시에 충족하고, 향후 사이버보안 관련 리콜 사유를 사전에 차단할 수 있다. IFU를 사용적합성 평가(model-name-B04) 테스트에 포함하면 IFU 이해도 미달로 인한 사용 오류 위험을 제거할 수 있다.

### 외주 개발 시 유의사항
라벨링 번역(다국어 IFU)을 외주 번역사에 위탁할 경우, 의료기기 전문 번역사를 사용해야 한다. 일반 번역사가 의료 용어를 잘못 번역하여 MFDS/EU 당국으로부터 IFU 재작성 요청을 받는 사례가 빈번하다. e-IFU(전자 IFU) 형태로 제공 시 종이 IFU 요청 시에도 즉시 제공 가능한 시스템을 구축해야 하며, 웹 주소(URL) 변경 시 기존 제출 사항과 불일치가 발생하지 않도록 영구 URL을 사용해야 한다.
